...
### Attempt at K-fold lambda method


```{r kfold_lambda_finding, echo = FALSE}

# further reduce metrics tabs for chinook (based on seasons affecting each cohort)
metrics_tab_screened_chinook = metrics_tab_screened[,keepers_ch]

mod_obj_co = kfold_cv(metrics_tab = metrics_tab_screened, y_val = y_val_coho, alpha = 1)
mod_co = mod_obj_co$mod; cv_co = mod_obj_co$cv


mod_obj_ch = kfold_cv(metrics_tab = metrics_tab_screened_chinook, y_val = y_val_chinook, alpha = 1)
mod_ch = mod_obj_ch$mod; cv_ch = mod_obj_ch$cv


coefs_co = get_pred_coefs(mod = mod_co, cv = cv_co, coef_digits = 3)
coefs_ch = get_pred_coefs(mod = mod_ch, cv = cv_ch)

# Plot cross validation for lasso regression
par(#mfrow = c(2,1), 
  mar = c(5,4,6,2))
plot(cv_co, main = paste0("Cross-validation, ",
                          yvlt$y_val_label[yvlt$y_val=="coho_smolt_per_fem"],
                          "; best lambda: ", round(cv_co$lambda.min, 3)))
par(#mfrow = c(2,1), 
  mar = c(5,4,6,2))
plot(cv_ch, main = paste0("Cross-validation, ",
                          yvlt$y_val_label[yvlt$y_val=="chinook_juvenile_abundance"],
                          "; best lambda: ", round(cv_ch$lambda.min, 3)))

flextable(data = coefs_co$coef)
flextable(data = coefs_ch$coef)
...
**revise: cross-validation in glmnet. number of folds to allow for grouping.**
...
# maybe coefficient table instead. Maybe that's all you need.
