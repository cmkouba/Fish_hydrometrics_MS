...
To make our model structure selection we weighed LASSO advantages (overfitting penalty, interpretable output) and disadvantages (assumes independent observations), MARSS advantages (built for working with small ecological time series records, accounts for autocorrelation) and disadvantages (less-interpretable output), and model performance against observed data. We also examined the following questions before making our selection:

* Do MARSS and LASSO models predict the same hydrologic metrics are of high importance?
* Do juvenile abundance and juveniles-per-spawner models predict the same hydrologic metrics are of high importance?
* For models of juvenile abundance, what is the relative importance (based on coefficients) of spawners versus hydrology?

MARSS models, single covariate (Tables \@ref(tab:MARSSSingleCovarTableCoho) and \@ref(tab:MARSSSingleCovarTableChinook)): hydrology order of importance is taken from the small-sample-size-corrected Akaike Information Criterion (AICc): the lower the AICc, the better the prediction based on that hydrologic metric. This also relates to coefficient value: the lower the AICc, the greater the coefficient assigned to the hydrologic metric in its single-covariate model.

MARSS models of juvenile abundance, two covariates (Tables \@ref(tab:MARSSFlowAndSpawnCovarTableCoho) and \@ref(tab:MARSSFlowAndSpawnCovarTableChinook)): used to ask, what is the relative influence of spawners and any single hydrologic metric? For the top ~4 best models, hydrology coeficients are equal to or greater than coefficients for spawners. For worse, higher-AICc models, spawner influence grows to be much greater than the hydrology.

MARSS single- and two-covariate models tended to perform better on coho than Chinook (Figures \@ref(fig:MARSSSingleCovars) and \@ref(fig:MARSSHydroPlusSpawn), possibly because Chinook records did not contain significant autocorrelation (Figure \@ref(fig:acfEcoRecords)). 
...
## Selected predictive models: LASSO regression of juveniles per spawner
...
For each species, the predictive model of ecological observations was based on the selected final lambda values, and refered to as the Hydrologic Benefit (HB) function. The HB function for each species is composed of an intercept and three non-zero coefficients, of which the first is approximately an order of magnitude greater than the trailing two (Tables \@ref(tab:coefTableCoho) and \@ref(tab:coefTableChinook)).
<!-- While many coefficients (as well as $R$ values in the correlation analysis) match the current understanding of local flow-ecology relationships, some do not (see Discussion). -->
