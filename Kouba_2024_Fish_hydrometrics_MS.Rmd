...
y_val_for_exp_fig = "chinook_spawner_abundance"
# fig_path = file.path(save_figs_here, paste0("Figure ",fig_i,".png"))
fig_path = file.path(save_figs_here, paste0(y_val_for_exp_fig," vs hydro vs time.pdf"))
if(save_imgs == TRUE){
  png(filename = fig_path, width = 7, height = 6, units = "in", res = res_dpi)

  eco_vs_time_figs = function(y_val = "chinook_spawner_abundance"){
  y_vals = metrics_tab[,y_val_for_exp_fig]
    # plot chinook abundance vs 
    for(i in 1:length(preds_for_fig)){
      pred = preds_for_fig[i]
      pred = metrics_tab[,"f1_recon_40"]
      
      plot(pred, y_vals, pch=19)
    }
