...
We used lasso regression [@ISLR; @RanstamCook2018] to assess the feasibility of predicting an ecological response using dozens of potential hydrologic predictor metrics. The objectives of the lasso exercise were to 1) perform predictor selection, i.e., empirically estimate which hydrologic flows were most related to coho and Chinook reproductive outcomes and 2) estimate the uncertainty of a predictive Hydrologic Benefit formula, using the selected set of predictors. Each step in the analysis is numbered and explained below.

## 1. Calculate predictors: Flow metrics to describe Scott River flow regime
...
Finally, based on input from local experts [@SRWC2023], we designed a metric to identify the presence of scouring flows. Scouring flows are storm events that can mobilize large amounts of sediment and either bury or wash away salmonid eggs []. 
...
## 2. Assemble response data: Ecological monitoring data
...
Based on these observations, two other combined metrics have historically been calculated and reported by regional agencies [e.g., @CDFW2023a]. These metrics use data from multiple years to capture multiple life stages for a given cohort:

4. The number of outmigrating coho smolt produced per spawning female (coho spf) and the outmigrating Chinook juveniles per adult metric (Chinook jpa).
...
## 3. Align predictor and response metrics with timing of species cohorts
...
smolt_year_hydro_tab = tabulate_hydro_by_affected_smolt_year(brood_years = 1999:2022,
...
After aligning the hydrologic predictors and ecological responses, we calculated Pearson correlation coefficients between  Calculate correlation matrix and remove values that don't make temporal sense (i.e., the wet season values in water year 2011 would not influence the number of spawners that arrived in fall of 2010)

### Rule out temporally impossible relationships

## 5. Select ecological response metrics


## 4. Predicting ecological outcomes for the species of concern

### Lasso regression analysis

With hydrologic metrics assigned to each salmon generation, indexed by brood-year and corresponding smolt year (*Supplemental Table 2*), we assessed the potential for hydrologic metrics to predict biological outcomes by performing lasso regression [@James2013; @RastamCook2018] between 40 hydrologic predictors and 7 ecological metrics (four for coho and three for Chinook).  

We used the R programming environment [@RManual] and the linear modeling function `glmnet()` [@Friedman2010] to perform variable selection using lasso regression. Lasso regression is a less-flexible method than the classic least squares regression, and is commonly used in high-dimensional data settings, where the number of possible predictors approaches or exceeds the number of observations [@James2013]. 
...
thereby enforcing that the sum of the all coefficient absolute values must be less than a fixed value $\lambda [@RastamCook2018]. In this formulation, higher values of lambda generally shrink the values of all coefficients to 0. [infinite lambda case]. The selection of an appropriate lambda value is critical, and is best done using cross-validation within the training dataset.
...
corr_matrix_fig_2(corr_matrix = corr_matrix_pearson_coho, 
                  thresholds = thresh_for_corr_fig)
...
if(save_imgs == TRUE){
  ...
  lasso_regression_plots(metrics_tab = metrics_tab, 
                         y_val = "chinook_juv_per_adult", remove_SY_metrics=T)
...
cor_tab_all_metrics = cor(metrics_tab, use = "pairwise.complete.obs")

  ry_rows = grepl(pattern = "RY", x = row.names(corr_matrix))
  sy_rows = grepl(pattern = "SY", x = row.names(corr_matrix))
