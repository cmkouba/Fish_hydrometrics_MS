...
**log-transform or Z-score flow metrics to standardize effect sizes?**
...
elim_pred_tab_col2 = c("How wet the wet season (year 1, as eggs and fry)",
                       ...
                       "Dry to wet transition timing (year 1, as eggs and fry)",
                       "Dry to wet transition timing (year 2, as rearing juv.)",
...
# fname_ch = paste( y_val_chinook,"- lambdas_and_rmse")
  ...
  if(log_transform_eco_metrics==F & zscore_flow_metrics ==F){
    fname_co = paste( y_val_coho,"- lambdas_and_rmse, no transform fmet or ecomet" )
    fname_ch = paste( y_val_chinook,"- lambdas_and_rmse, no transform fmet or ecomet" )
  }
  if(log_transform_eco_metrics==T & zscore_flow_metrics ==T){
    fname_co = paste( y_val_coho,"- lambdas_and_rmse, zscore fmet T, log ecomet T" )
    fname_ch = paste( y_val_chinook,"- lambdas_and_rmse, zscore fmet T, log ecomet T" )
  }


if(file.exists( file.path(data_dir,paste0(fname_co,".csv" )))){
  l_and_r_co = read.csv(file.path(data_dir,paste0(fname_co,".csv")))
...
if(file.exists( file.path(data_dir,paste0(fname_ch,".csv" )))){
  l_and_r_ch = read.csv(file.path(data_dir,paste0(fname_ch,".csv")))
...
if(log_transform_eco_metrics == T & zscore_flow_metrics == T) {
  if(y_val_coho=="coho_smolt_abun_est") {lam_co = 0.1}
  if(y_val_chinook == "chinook_juvenile_abundance"){lam_ch = 0.1}
  if(y_val_coho=="coho_smolt_per_fem") {lam_co = .2}
  if(y_val_chinook == "chinook_juv_per_adult"){lam_ch = .1}
}
...
lasso_results_co = lasso_regression_plots_and_tabs(
  ...
  show_plot = save_imgs,
  log_transform_eco_metrics = log_transform_eco_metrics, 
  zscore_flow_metrics = zscore_flow_metrics) 
...
metrics_tab_long = metrics_tab_long[,colnames(metrics_tab_screened)]
