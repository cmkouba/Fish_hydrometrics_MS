...
# Remove the log metrics (TEMPORARY; if it works just remove from table function)
exclude_these = grepl(pattern = "log_",x=colnames(metrics_tab))
metrics_tab = metrics_tab[!exclude_these,]; metrics_tab = metrics_tab[,!exclude_these]
...
diag(corr_tab_screener) = NA

exclude_these = colnames(corr_tab_screener) %in% c("brood_year","smolt_year")
...
corr_tab_screened = corr_tab_long[ !is.na(corr_tab_long$value)
                                   & corr_tab_long$value > high_corr_threshold,]


greater_than_thresh_finder = apply(X= corr_tab_screener, MARGIN = 1,
                                function(x){sum(abs(x) >= high_corr_threshold,
                                                na.rm=T) > 0})
pred_subset = row.names(corr_tab_screener)[greater_than_thresh_finder] 

corr_tab_screened = corr_tab_screener[colnames(corr_tab_screener) %in% pred_subset,]
