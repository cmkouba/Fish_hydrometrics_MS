...
Taking advantage of a nearly two-decade ecological monitoring dataset, we used lasso regression to build predictive models of coho and Chinook salmon reproductive success using hydrologic metrics as predictors. To control for cohort effects, we chose normalized ecological response metrics for coho and Chinook (number of outmigrating smolt per spawning adult or spawning adult female). For both species, we calculated optimal predictive models for thousands of 50% subsamples of the datasets, and tested them on the remaining observations. We selected a lambda value based on an average relative test error threshold of 1.0. These lambda values were used to calculate a final predictive model, or Hydrologic Benefit function, using the full dataset for each species. The metrics contributing the most to predicted coho spf values are occur during the window of their parents' spawning and, to a lesser extent, in the spring and fall of their year of rearing in freshwater. This supports an interpretation that spawning conditions may exert a significant influence on the mortality rates of the hatching juveniles.
...
<!-- ```{r select_eco_metric_1, echo = F, warning =F, comment = F} -->
...
<!-- # corr_tab_2[corr_tab_2==1] = NA # remove self-correlation values of 1 # this is done in a later step -->

<!-- # Remove correlation values with a sample size of < threshold value (10?) -->
<!-- min_pairs = 10 -->

<!-- calc_samp_size = function(mt){ -->
<!--   out_tab = as.data.frame(matrix(data=NA, nrow = ncol(mt), ncol =ncol(mt))) -->
<!--   colnames(out_tab) = colnames(mt) -->
<!--   row.names(out_tab) = colnames(mt) -->

<!--   ij_combos = t(combn(x=1:ncol(mt), m=2)) -->
<!--   for(k in 1:nrow(ij_combos)){ -->
<!--     i=ij_combos[k,1]; j=ij_combos[k,2] -->
<!--     out_tab[i,j] = sum(!is.na(mt[,i] * mt[,j])) -->
<!--     out_tab[j,i] = sum(!is.na(mt[,i] * mt[,j])) -->
<!--   } -->
<!--   return(out_tab) -->
<!-- } -->

<!-- samp_size_tab = calc_samp_size(mt = metrics_tab2) -->
<!-- keep_corr_tab = samp_size_tab/samp_size_tab -->
<!-- keep_corr_tab[samp_size_tab < min_pairs] = NA -->

<!-- # Multiply the correlations tab by keep_corr to zero out  -->
<!-- # the corr coeffs with insufficient sample size -->
<!-- corr_tab_2 = corr_tab_2 * keep_corr_tab -->
...
<!-- ```{r select_eco_metric_2_diagnostics, echo = F, warning =F, comment = F} -->


<!-- show_eco_picker_diagnostics = F -->

<!-- if(show_eco_picker_diagnostics==T){ -->
...
<!-- max_pred = 20 -->
<!-- out_tab = as.data.frame(matrix(data=NA, nrow = length(y_vals), ncol=max_pred)) -->
<!-- colnames(out_tab)=paste0("preds_",1:max_pred) -->
<!-- row.names(out_tab)=y_vals -->

<!-- for(num_top_pred in 1:max_pred){ -->
<!--   # num_top_pred =3 -->
...
<!-- # print(paste("Num pred:",num_top_pred,"; Chinook yval:",y_val_chinook, "; Coho yval:", y_val_coho)) -->
<!-- # print(paste("Num pred:",num_top_pred)) -->
<!-- # print(corr_co) -->
<!-- # print(corr_ch) -->
<!-- } -->
...
<!-- # plot for Chinook -->
<!-- plot(x= 1:max_pred, y= out_tab[y_vals[1],], type = "o", col = 1, ylim=c(0.3,1), pch=20) -->

<!-- for(i in 2:3){#length(y_vals)){ -->
<!--   lines(x=1:max_pred, y=out_tab[y_vals[i],], col=i) -->
<!--   points(x=1:max_pred, y=out_tab[y_vals[i],], col=i, pch=20) -->
<!--   } -->
<!-- legend(x="topright", legend = y_vals, col = 1:length(y_vals), lwd=2) -->
<!-- grid() -->

<!-- # plot for coho -->
<!-- plot(x= 1:max_pred, y= out_tab[y_vals[4],], type = "o", col = 4, ylim=c(0.3,1), pch=20) -->

<!-- for(i in 5:length(y_vals)){ -->
<!--   lines(x=1:max_pred, y=out_tab[y_vals[i],], col=i) -->
<!--   points(x=1:max_pred, y=out_tab[y_vals[i],], col=i, pch=20) -->
<!--   } -->
<!-- legend(x="topright", legend = y_vals, col = 1:length(y_vals), lwd=2) -->
...
We used 100 lambda values within the optimal range (identified in step 6) to generate a series of regression models to predict ecological outcomes for coho and Chinook. As is typical in lasso regression, coefficient values in these models approach 0 as the lambda value increases (Figures \@ref(fig:lassoResultsCoho) and \@ref(fig:lassoResultsChinook), top panel).

Each model explains a fraction of the deviation of the observed values from the mean. This fraction ranges from 0% of (with all coefficients set to 0) to 100% of deviation. The number of non-0 coefficients explaining 100% of variation is 12 and 24 for coho and Chinook respectively. This matches the general rule of thumb that with high-dimensional data, it is often possible to explain 100% of deviation by incorporating data from a number of predictors that approaches or exceeds the number of observations [@James2013]. 
...
As mentioned above, to find the range of potential optimal lambda values and estimate the test error within this range, we divided the observations randomly into different sets of training and test data (1,716 sets for coho spf and 10,000 for Chinook jpa). Each of the regression models produced from a training subset was associated with an order of appearance of predictors (similar to the results for the full dataset in Figures \@ref(fig:lassoResultsCoho) and \@ref(fig:lassoResultsChinook) [top panel], and listed in Tables \@ref(tab:predAppearTabCoho) and \@ref(tab:predAppearTabChinook)).
