...
# spawner_records = yvlt$y_val[grepl(pattern = "spawner", x = yvlt$y_val)]
...
Taking advantage of a nearly two-decade ecological monitoring dataset and long-term stream gauge measurements, we first examined hydrological-ecological correlations, then implemented four different statistical modeling exercises. Of these, we selected lasso regression, using hydrologic metrics to predict an outmigrant-to-spawner ratio, to generate a final predictive model for each species. This model predicts units of $log_{10}(outmigrant/spawner)$ and is referred to as a Hydrologic Benefit function.  In the lasso regressions, to balance the explanatory power of the models with the risk of overfitting, we used k-fold cross-validation to find the lowest-error value of the tuning parameter lambda. 

Correlation coefficients indicate that hydrologic factors and spawner abundance both exert influence on juvenile fish production. In low-error lasso regression models of this ratio, hydrology could explain a greater degree of variance for Chinook than for coho (approximately 60% and 40%, respectively). The single most important hydrologic metric was earlier river reconnection during parents' spawning for coho, and lower wet season baseflow for Chinook. This suggests that conditions during their parents' spawning or as eggs and hatchlings may exert a significant influence on the mortality rates of the hatching juveniles in this study area.

The simplicity of this approach for empirically identifying hydrologic metrics with the highest ecological benefit for a threatened species may be useful in other watersheds (where sufficient ecological data records are available), and could be a step towards evaluating trade-offs and supporting water management decisions in human-altered novel ecosystems.
...
2. In statistical models, hydrology could explain a greater degree of variance in relative Chinook reproduction than in coho (60% versus 40%), suggesting that Chinook outcomes may be more sensitive to flow than coho.
3. The most important single hydrologic metric was earlier river reconnection during parents' spawning for coho, and lower wet season median baseflow for Chinook. This suggests that conditions during their parents' spawning or as eggs and hatchlings may exert a significant influence on the mortality rates of the hatching juveniles in this study area.
...
Reconciliation ecology posits that some human-impacted ecosystems should be considered irrevocably-altered, "novel" systems [@MoyleNOVEL2014;@YarnellEtAlFunctional2015], with their own specific management concerns. To implement this philosophy, rather than working to restore novel ecosystems to pre-human conditions, a natural resource manager would actively manage biodiversity in human-altered landscapes as a co-equal goal with extracting and cultivating natural resources to provide for human material needs [e.g., @RobertsonSwintonReconciling2005; @ArthingtonEtAlTEMPORARY2014; @AcremanEtAlEnvironmental2014; @YarnellEtAlFunctional2020]. 
...
<!-- [@HorneEtAlOptimization2016] review of methods for optimizing  dam releases for env flows. No one has actually used optimization in their management. "Env decisions poorly informed by env research" -->

<!-- [@JagerThinking2014] habitat simulation and population/growth modeling in Central Valley chinook. -->

<!-- [@JagerRoseDesigning2003] Used flow to predict fish recruitment. Optimal flow regimes different for different env objectives?   -->

<!-- [@TurnerStewardsonHydrologic2014] Does hydro alteration mean alteration of hydraulic microhabitat environments? "Modelling flow-biota relationships in a regional context is limited by the inadequacy of streamflow statistics to model ecologically significant hydraulic function." Three levels of specificity: site-specific hydraulics (non-linearity in eco response to flow); eco responses to flow across multiple sites (hydraulic response to a given flow confounded by geomorph) ; flow used as proxy for hydraulics/eco metrics (this is just bad).  Depending on geomorph, "the same flow alteration can produce quite different changes in hydraulic env." -->

<!-- [@Grantham2020] PPIC report. describes CEFF. Calls for CA to use it. Advocates ecosystem water budget with senior water rights. -->

<!-- [@Stein2021]. Developed CEFF. Interagency Workgroup. Example of state-scale e-flows recommendation rather than watershed-specific flow regime. Tiered framework: full consideration of ecosystem needs before deciding trade-offs with other water uses. -->




<!-- [@YarnellEtAlFunctional2015]. Preserving functions in modified riverscapes, examples in rivers in western US. "Managing rivers in a nonstationary world requires management strategies and policies that focus on preserving key functions and processes to sustain the dynamically evolving nature of river ecosystems" -->
<!-- [@Arthington2012]. Environmental Flows: Saving Rivers in the Third Millenium. Story of e flows development. -->
<!-- "functional flows" term proposed in Escobar-Arias and Pasternack 2010 -->
<!-- [@KelsonCarlsonPrecipitation2019]. Dry and wet year diffs in streamflows in the S Fork Eel do not impact out-migration timing of steelhead (maybe because it's such good habitat).  -->


<!-- [@SellheimEtAlInformed2020]. Sac River Chinoook real time adaptive flow mgmt: Two prescribed flow pulses reduced 1) fish egg stranding,  and 2) high juvenile fish densities in 2013-2014. -->
<!-- [@Bellido-LeivaEtAlModeling2021]. Simple population model plus hydraulics plus bioenergetics in different habitats mostly reproduced outmigrant numbers and timing.  -->


<!--  [@MichelEtAlNonlinear2021]. Flow-ecology relationships. Previous work: more Central Valley flow can cause more juvenile fish survival, but nonlinearities not explored. This study developed a flow-survival relationship in different water years (2013-2019). Survival % numbers (min, mean, high thresholds): Below min: 3%; Min-mean: 19%; mean-high: 50% (also shortest travel times out); above high: 35% survival. Then, calculated survival rates in redesigned hydrographs. Focused on one functional flow (spring pulse), one lifestage and function (outmigration of juveniles) and had great eco data (PIT tagged transit times and survival rates). -->

<!-- [@BarbourEtAlOptimisation2016]. Tradeoffs between conflicting objectives. 4 challenges: ID and quantify ecosystem objectives; spec obj and mgmt. alts in an optimization framework; and testing predictions against observations. Practices for defining obj functions. Two types of obj fns: achieving hydro metrics and targeting eco needs directly. Level of specificity affects quantifiability. Involving stakeholders is important. Species-preference/eco needs methods: need to pick an appropriate umbrella/keystone species. Good history of Functional Flows and utility. Optimization methods: metaheuristics (fuzzier?) and classical methods like linear programming. Obj fn definitions: 1) max/min totals or avgs (tends to prioritize outlier "good" periods instead of longer periods avoiding, e.g., critical lows); 2) max/min diff between actual and target values; 3) constraints; 4) max a min (avoid worst case conditions)/min a max. also: no one checks to see if their model bears out in practice. -->

<!-- [@NislowArmstrongLifehistorybased2012] "fundamental interactions between flow and water temperature must be incorporated into the robust models ultimately required for science-based management" -->
<!--  [@JagerSmithSustainable2008]. Need more prediction of ecosystem responses to flow. -->
<!-- [@NullLundFISH2012]. Shasta R. Scenarios: relocating major diversion, increasing riparian shading, higher instream flow, coolwater spring, removing a dam -->
<!-- Other notes: -->
<!-- Regulated rivers e flows started at: single minimum in-stream flow during low-flow periods. -->
<!-- Utility of hydrologic-only methods. -->
<!-- Species-specific study versus attempting to preserve functions for the whole ecosystem: "umbrella species" effect for some species? -->
...
## Step 6. Generate predictive models 

```{r statsMethodsTable, echo = F, warning =F, results = 'asis'}

stats_tab = data.frame(Predicts = c("juveniles per adult", "juvenile abundance", "juveniles per adult", "juvenile abundance"),
                       Method = c("Lasso", "Lasso", "MARSS", "MARSS"),
                         Predictors = c("hydrologic metrics", "hydrologic metrics and spawner abundance", 
                                        "hydrologic metrics", "hydrologic metrics and spawner abundance"),
                         Location = c("Main text (selected as primary method)", rep("Supplement",3)))

stats_tab = stats_tab[c(1,3,2,4),]
...
### Lasso Regression, general approach
...
findings_tab = data.frame(Predicts = c("juveniles per adult", "juvenile abundance", "juveniles per adult", "juvenile abundance"),
                       Method = c("Lasso", "Lasso", "MARSS", "MARSS"),
                         # Predictors = c("hydrologic metrics", "hydrologic metrics and spawner abundance", 
                         #                "hydrologic metrics", "hydrologic metrics and spawner abundance"),
                         # Location = c("Main text (Selected as primary method)", rep("Supplement",3)),
                       MostImportantHydroCoho = c("Earlier first fall reconnection, longer first wet season, smaller second fall flow increase, faster second spring recession",
                                                  "Larger first fall flow increase, smaller second fall flow increase, slower first spring recession",
                                                  "Earlier fall reconnection, larger first fall flow increase, greater first wet season baseflow",
                                                  "Larger first fall flow increase, earlier fall reconnection, greater dry season high flows"),
                       MostImportantHydroChinook = c("Lower wet season baseflow, slower max. spring recession rate, longer wet season",
                                                     "Lower wet season baseflow, (spawner abundance), slower max. spring recession, longer wet season",
                                                     "Greater dry season high flows, slower max. spring recession rate, earlier fall reconnection",
                                                     "Slower max. spring recession rate, lower wet season baseflow, earlier fall reconnection"),
                       
                       #                        MostImportantHydroCoho = c("f1_recon_120, w1_Wet_BFL_Dur, f2_FA_Dif_num, s2_SP_ROC",
                       #                            "f1_FA_Dif_num, f2_FA_Dif_num, s1_SP_ROC",
                       #                            "f1_recon_120, f1_FA_Dif_num, w1_Wet_BFL_Mag_50, d1_DS_Mag_90",
                       #                            "f1_FA_Dif_num, f1_recon_120, d1_DS_Mag_90, w1_Wet_BFL_Mag_50"),
                       # MostImportantHydroChinook = c("w1_Wet_BFL_Mag_50, s1_SP_ROC_Max, w1_Wet_BFL_Dur",
                       #                               "w1_Wet_BFL_Mag_50, (spawner abundance), s1_SP_ROC_Max, w1_Wet_BFL_Dur",
                       #                               "d1_DS_Mag_90, s1_SP_ROC_Max, f1_recon_120, w1_Wet_BFL_Dur",
                       #                               "s1_SP_ROC_Max, w1_Wet_BFL_Mag_50, f1_recon_120, d1_DS_Mag_50"), 
                       H2SRatioCo= c("(untested)", "Greater hydro influence; Hydro-to-Spawner >8x","(untested)","~Equivalent influence (Hydro-to-Spawner of 0.7 to 1.5)"),
                                              H2SRatioCh= c("(untested)", "Greater hydro influence; Hydro-to-Spawner >2x","(untested)","Greater hydro influence; Hydro-to-Spawner of 1.7 to 6.3"))

findings_tab = findings_tab[c(1,3,2,4),]

caption_text = "Description of four different techniques for modeling two types ecological outcomes using hydrologic metrics (and available ecological data, if applicable)."

ft_ft = flextable(data=findings_tab)
ft_ft = set_caption(ft_ft, caption = caption_text)
ft_ft = flextable::width(ft_ft,j=1,  width = 0.9)
ft_ft = flextable::width(ft_ft,j=2,  width = 0.9)
ft_ft = flextable::width(ft_ft,j=3,  width = 1.8)
ft_ft = flextable::width(ft_ft,j=4,  width = 1.8)
ft_ft = flextable::width(ft_ft,j=5,  width = 1.3)
ft_ft = flextable::width(ft_ft,j=6,  width = 1.3)
...
ft_ft = flextable::set_header_labels(ft_ft,
                                           values = list(
                                             Predicts ="",
                                             Method = "",
                                             # MostImportantHydroCoho = "Important Hydro. for Coho",
                                             # MostImportantHydroChinook = "Important Hydro. for Chinook",
                                             MostImportantHydroCoho = "Coho",
                                             MostImportantHydroChinook = "Chinook",
                                             # H2SRatioCo = "Coef. Ratios, Hydro-to-Spawner",
                                             H2SRatioCo = "Coho", H2SRatioCh= "Chinook"))

ft_ft = add_header_row(ft_ft, values = c("Predicts","Method","Most Important Hydrologic Metrics","Hydrologic vs. Spawner Influence (Coef. Ratios)"), colwidths = c(1,1,2,2), top = T)
ft_ft
```
...
# old table without accounting for spawners
# if(y_val_coho == "coho_smolt_abun_est" & log_transform_eco_metrics == T & 
#    zscore_flow_metrics == T ){
#   # coefs_interp$f1_recon_120 = "Earlier fall reconnection (during parents' spawning)"
#   coefs_interp$f2_FA_Dif_num = "Smaller fall flow increase (as juvenile fish)"
...
if(y_val_chinook == "chinook_juvenile_abundance" & predict_eco == "juvenile abundance, hydro and spawners"){
...
if(y_val_chinook == "chinook_juvenile_abundance" & predict_eco == "juvenile abundance, hydro only"){
  coefs_ch_interp$w1_Wet_BFL_Mag_50 = "Lower wet season baseflows (first wet season, as eggs and fry)"
}
...
y_text_co = paste0("Contrib. to HB value (log10 of ",
                   yvlt$y_val_title[
...
y_text_ch = paste0("Contrib. to HB value (log10 of ",
                   yvlt$y_val_title[
    ...
    if(predict_eco == "juveniles per spawner"){
      hbf_bar_co = ggplot(data = bd_melt_co, 
                          aes(x = `Brood Year`, 
                              ...
                              y = `Contrib. to HB value (log10 of coho spf)`,
                              fill = variable)) + 
    ...
    if(predict_eco %in% c("juvenile abundance, hydro and spawners",
      ...
      hbf_bar_co = ggplot(data = bd_melt_co, 
                          aes(x = `Brood Year`, 
                              ...
                              y = `Contrib. to HB value (log10 of coho smolt abundance)`,
                              fill = variable)) + 
        geom_bar(stat = "identity") + 
        scale_fill_brewer(palette = "Spectral") +
        theme(legend.position = "right", legend.direction = "vertical")
    ...
    if(predict_eco == "juveniles per spawner"){
      hbf_bar_ch = ggplot(data = bd_melt_ch, 
                          aes(x = `Brood Year`, 
                              ...
                              y = `Contrib. to HB value (log10 of Chinook jpa)`,
                              fill = variable)) + 
        geom_bar(stat = "identity") + 
        scale_fill_brewer(palette = "Spectral") +
        theme(legend.position = "right", legend.direction = "vertical")
    ...
    if(predict_eco %in% c("juvenile abundance, hydro and spawners",
      ...
      hbf_bar_ch = ggplot(data = bd_melt_ch, 
                          aes(x = `Brood Year`, 
                              ...
                              y = `Contrib. to HB value (log10 of Chinook juv. abundance)`,
                              fill = variable)) + 
        geom_bar(stat = "identity") + 
        scale_fill_brewer(palette = "Spectral") +
        theme(legend.position = "right", legend.direction = "vertical")
