...
In many rural areas of arid and semi-arid regions, balancing agricultural and environmental water needs is a key challenge facing resource managers. This is complicated by the tendency for the water needs of cultivated crops to be better understood than those of aquatic ecosystems. In particular, the timing and magnitude of flow needed to sustain key ecological functions remain poorly quantified in many regions. This work aims to quantify hydrologic conditions that support persistence of key ecosystem species using a functional flows framework, not in the context of a prescribed flow regime, but in terms of which features of the hydrograph are empirically correlated with ecological outcomes. We use the (*Oncorhynchus kisutch*) and Chinook (*Oncorhynchus tshawytscha*) salmon run in Scott Valley, a 2,109 km2 undammed rural watershed in northern California, USA, as a case study.

Taking advantage of a nearly two-decade ecological monitoring dataset and long-term stream gauge measurements, we compared eight total modeling exercises: we used two  regression methods (lasso and MARSS) to build predictive models of two different units of salmon outcome (outmigrating juvenile abundance, based on hydrologic metrics and parental spawner abundance; or an outmigrant-to-parental-spawner ratio, based on hydrologic metrics only) for two key species, coho and Chinook salmon. In the lasso regressions, to balance the explanatory power of the models with the risk of overfitting, we used k-fold cross-validation to find the lowest-error value of the tuning parameter lambda. To assess the utility of the MARSS regressions we calculated the autocorrelation function of each ecological data time series.

We selected lasso regression of outmigrant-to-spawner ratio to calculate a final predictive model, referred to as a Hydrologic Benefit function, for each species. In low-error lasso regression models, hydrology could explain a greater degree of variance in this ratio for Chinook than for coho. The most important hydrologic metrics associated with higher relative reproduction were an earlier river reconnection after the dry season of parents' spawning, a longer duration of the first wet season, and a smaller fall pulse flow as oversummering juveniles for coho; and lower wet season baseflow, slower maximum spring recession rate, and a longer wet season duration for Chinook. The large coefficients calculated for fall river reconnection timing for coho, and lower wet season baseflows for Chinook, suggest that conditions during the window of their parents' spawning or as eggs and hatchlings, supports an interpretation that spawning conditions may exert a significant influence on the mortality rates of the hatching juveniles.
...
Specifically, results indicate that the timing of stream connection and disconnection each year relate to relative coho reproduction and the length of the wet season relates to spawning success." (or whatever is most accurate).
...
One method for estimating ecological water needs is the functional flows framework [@YarnellEtAlFunctional2015; @GranthamEtAl2020Making] [@PoffEtAlNatural1997; @PoffEtAlEcological2010]. Functional flow metrics are used to quantify potential ecological services provided by river flow in terms of flowrate magnitude, timing, frequency, and duration in distinct seasons of a water year, where water year is here defined to begin on October 1 of the year preceding the calendar year of the same number (i.e., water year 2020 begins on October 1, 2019). Recent work has refined these metrics specific to California hydrology [@SteinEtAlCalifornia2021] and made the metric-calculating algorithms publicly available [@YarnellEtAlFunctional2020; @PattersonEtAlHydrologic2020].
...
#remove spawners from lasso exercise if not using as a predictor
if(predict_eco == "juveniles per spawner"){keepers_co = keepers_co[!(keepers_co %in% spawner_z_records)]}
...
ft_ft = flextable::width(ft_ft,j=3,  width = 1.9)
ft_ft = flextable::width(ft_ft,j=4,  width = 1.9)
ft_ft = flextable::width(ft_ft,j=5,  width = 1.4)
ft_ft = flextable::width(ft_ft,j=6,  width = 1.4)
