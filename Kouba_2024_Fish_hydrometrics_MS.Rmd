...
mod_obj_ch = kfold_cv(metrics_tab = metrics_tab_screened_chinook, alpha = 1, 
                      y_val = y_val_chinook)
...
par(mfrow = c(3,1))
plot(cv_ch)
plot(log(mod_ch$lambda), deviance(mod_ch) / mod_ch$nulldev, type = "l",
     xlab = "Log Lambda", ylab = "Fraction of null deviance explained")
abline(v=log(cv_ch$lambda.min), lty = 2, lwd = 2, col = "brown")
plot(mod_ch, xvar = "lambda")
abline(v=log(cv_ch$lambda.min), lty = 2, lwd = 2, col = "brown")
...
coefs_co_vals = c(round(coefs_co$int_and_coefs$coef, 1))
...
coefs_ch_ft = data.frame(Predictor = c(names(coefs_ch_interp)),
                         Value = as.vector(round(coefs_ch_vals,0)),
