...
if(log_transform_eco_metrics == T & zscore_flow_metrics == T) {
  if(y_val_coho=="coho_smolt_abun_est") {lam_co = 0.2}
  if(y_val_chinook == "chinook_juvenile_abundance"){lam_ch = 0.12}
  if(y_val_coho=="coho_smolt_per_fem") {lam_co = .075}
  if(y_val_chinook == "chinook_juv_per_adult"){lam_ch = .075}
}
...
if(y_val_coho == "coho_smolt_per_fem" & log_transform_eco_metrics == F & zscore_flow_metrics == F){
  ...
  coefs_interp$s1_SP_ROC_Max = "Max spring recession rate of change (as juvenile fish)"
  ...
  coefs_interp$s1_SP_ROC = "Slower rate of change, second spring recession (as juvenile fish)"
...
if(y_val_coho == "coho_smolt_abun_est" & log_transform_eco_metrics == T & zscore_flow_metrics == T){
    coefs_interp$d1_DS_Mag_50 = "Higher dry season flow (before parents' spawning)"
    coefs_interp$w1_Wet_BFL_Dur = "Higher wet season baseflow (as eggs and fry)"
    coefs_interp$s1_SP_ROC = "Slower spring recession rate of change (as juvenile fish)"
    coefs_interp$w2_Wet_BFL_Dur = "Shorter wet season baseflow duration (as overwintering juveniles)"
    coefs_interp$s2_SP_ROC = "Faster spring recession rate of change (as juvenile fish)"
...
coefs_ch_interp_names = names(coefs_ch)
coefs_ch_interp = as.character(coefs_ch)
names(coefs_ch_interp) = coefs_ch_interp_names
