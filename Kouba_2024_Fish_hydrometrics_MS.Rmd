...
**Key**
*Wet Season*:	Functional Flow season descriptor
*Dec-Mar*:	Typical, approximate date range for season to occur in Scott Valley
*w1*:	Abbreviated season identifier (i.e., first wet season experienced by a salmon cohort)
*Egg, alevin, fry*:	Life stage in which salmon experienced this season
...
while(length(collin_preds)>0){
  ...
  if(loop==6){selected_pred[[loop]] = "f1_FA_Dif_num"} # Fall pulse magnitude during spawning (diff has higher sample size)
  if(loop==7){selected_pred[[loop]] = "f2_FA_Dif_num"} # Fall pulse magnitude (rearing juv) (diff has higher sample size)
...
# final lambda vals
if(y_val_coho=="coho_smolt_per_fem") {lam_co = 7}#10}
if(y_val_chinook == "chinook_juv_per_adult"){lam_ch = 37}#50}
...
if(y_val_coho == "coho_smolt_per_fem"){
  ...
  coefs_interp$f1_recon_120 = "Earlier reconnection (120 cfs, fall of parents' spawning)"
...
if(y_val_chinook == "chinook_juv_per_adult"){
  ...
  coefs_interp$w1_Wet_BFL_Dur = "Longer wet season (as eggs and fry)"
  coefs_interp$w1_Wet_BFL_Mag_50 = "Lower wet season flows (first wet season, as eggs and fry)"
