...
y_val_label_tab = data.frame(y_val = c("chinook_spawner_abundance",
                             ...
                             y_val_label = c("Num. Chinook spawners (escapement)",
                                             ...
                                             "% coho smolt survival",
                                             "Num. obs. coho redds"))
non_preds = c("brood_year", "smolt_year", y_val_label_tab$y_val)
...
if(save_imgs == TRUE){
  ...
  pred_tab_co = lasso_regression_plots(metrics_tab = metrics_tab, 
                                       y_val = y_val_coho,
                                       remove_SY_metrics = T, # for redds or spawners
                                       y_val_label_tab=y_val_label_tab)
...
if(y_val_coho=="percent_coho_smolt_survival") {lam_co = 3.5} 
...
if(y_val_chinook == "chinook_juv_per_adult"){lam_ch = 21} # for Chinook jpa
...
if(save_imgs == TRUE){
  png(filename = fig_path, width = 7, height = 8, units = "in", res = 300)
  y_axis_label =  y_val_label_tab$y_val_label[y_val_label_tab$y_val==y_val_coho]
  par(mfrow=c(2,1))
  # Panel 1. Coho
  ...
  y_axis_label =  y_val_label_tab$y_val_label[y_val_label_tab$y_val==y_val_chinook] 
  hbf_over_time_fig(metrics_tab = metrics_tab,
                    hbf_tab = hbf_chinook, 
                    y_val = y_val_chinook,
                    y_val_axis = y_axis_label) 
...
if(length(comp_names_co)>0){
  ...
  colnames(bd_melt_co)[3] = paste0("Contribution to Hyd. Ben. value (",
                                  y_val_label_tab$y_val_label[
                                    y_val_label_tab$y_val==y_val_coho],")")
}
...
if(length(comp_names_ch)>0){
  ...
  colnames(bd_melt_ch)[3] = paste0("Contribution to Hyd. Ben. value (",
                                  y_val_label_tab$y_val_label[
                                    y_val_label_tab$y_val==y_val_chinook],")")
}
# Bar charrrt
...
if(save_imgs==T){
  if(length(comp_names_co)>0){
    ...
    hbf_bar_co = ggplot(data = bd_melt_co, 
                        aes(x = `Brood Year`, 
                            y = `Contribution to Hyd. Ben. value (% coho smolt survival)`,
                            fill = variable)) + 
  ...
  if(length(comp_names_ch)>0){
        ...
        hbf_bar_ch = ggplot(data = bd_melt_ch, 
                        aes(x = `Brood Year`, 
                            y = `Contribution to Hyd. Ben. value (Chinook juv. per adult)`,
                            fill = variable)) + 
      geom_bar(stat = "identity") + 
      scale_fill_brewer(palette = "Spectral") +
      theme(legend.position = "right", legend.direction = "vertical")
