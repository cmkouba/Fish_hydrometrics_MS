...
hbf_coho = get_hbf_from_lasso_mod(metrics_tab,
                                  y_val = y_val_coho,
                                  lambda_val = lam_co,
                                  remove_extra_recon_thresholds = F,
                                  remove_SY_metrics = F)

coho_coefs = as.data.frame(t(as.matrix(coef(hbf_coho))))
# coef_names = row.names(coef(hbf_coho))
#removing the intercept will make the row names unusable

int_picker = 1
intercept = coho_coefs[int_picker]
coho_coefs = coho_coefs[-int_picker]
# coef_names = coef_names[-int_picker]

# CURRENTLY HERE. WRITE FUNCTION TO APPLY COEFFICIENTS TO ANNUAL VALUES
# TO CALCULATE ANNUAL HB VALUES

output_tab=metrics_tab
output_tab$hb_i = NA
# match metrics tab to coefficients
term_matcher = match( colnames(coho_coefs), colnames(metrics_tab))
output_tab[,term_matcher] = 0


for(i in 1:nrow(metrics_tab)){
  by_vals = metrics_tab[i,]
  hb_components = by_vals [term_matcher] * coho_coefs
  output_tab[term_matcher,i] = hb_components
  output_tab$hb_i[i] = sum(intercept, hb_components, na.rm=T)
}



# hbf_tab = calc_hbf_tab_may2024(flow_tab_for_hbf = fj_flow,
#                                weights = mean_row,
#                                ch1_hbftab = T,
