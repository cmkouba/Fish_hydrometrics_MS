...
# metrics_tab_screened and _chinook

# calculate lasso regression model and cross-validation objects
# (lasso: alpha = 1; ridge: alpha = 0)
mod_obj_co = kfold_cv(metrics_tab = metrics_tab_screened, y_val = y_val_coho, alpha = 0)
mod_co = mod_obj_co$mod; cv_co = mod_obj_co$cv

mod_obj_ch = kfold_cv(metrics_tab = metrics_tab_screened_chinook, alpha = 0,
                      y_val = y_val_chinook)
...
coefs_co = get_pred_coefs(mod = mod_co, cv = cv_co, coef_digits = 1)
...
plot(cv_co, main = paste0("Cross-validation, ",
                          yvlt$y_val_label[yvlt$y_val==y_val_coho],
                          "; best lambda: ", round(cv_co$lambda.min, 3)))
par(#mfrow = c(2,1),
  mar = c(5,4,6,2))
plot(cv_ch, main = paste0("Cross-validation, ",
                          yvlt$y_val_label[yvlt$y_val==y_val_chinook],
                          # yvlt$y_val_label[yvlt$y_val=="chinook_juv_per_adult"],
...
# run main .Rmd script through metrics_tab_screened
...
for(i in 1:nrow(yvlt)){ # for each ecological time series,
