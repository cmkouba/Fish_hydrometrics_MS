...
## Lasso Regression, juveniles per spawner

```{r kfold_lambda_juv_per_adult, echo = FALSE}
...
caption_text = "Values for the intercept and coefficient terms in the Hydrologic Benefit function for estimated coho smolts per female spawner, including a description of which phenomena are associated with higher ecological outcome values."
...
colnames(results_y_co) = c("Covariate", "AICc", "Coefficient")

caption_text = paste0("Characteristics of MARSS models of ", yvlt$y_val_title[yvlt$y_val==y_val_coho], " using one hydrologic metric as the single covariate in each model.")

coco_tab_ft = flextable(data=results_y_co)
coco_tab_ft = set_caption(coco_tab_ft, caption = caption_text)
coco_tab_ft = flextable::width(coco_tab_ft,j=1,  width = 1.5)
coco_tab_ft = flextable::width(coco_tab_ft,j=2,  width = 1)
coco_tab_ft = flextable::width(coco_tab_ft,j=3,  width = 3)
...
colnames(results_y_ch) = c("Covariate", "AICc", "Coefficient")


caption_text = paste0("Characteristics of MARSS models of ", yvlt$y_val_title[yvlt$y_val==y_val_chinook], " using one hydrologic metric as the single covariate in each model.")

coch_tab_ft = flextable(data=results_y_ch)
coch_tab_ft = set_caption(coch_tab_ft, caption = caption_text)
coch_tab_ft = flextable::width(coch_tab_ft,j=1,  width = 1.5)
coch_tab_ft = flextable::width(coch_tab_ft,j=2,  width = 1)
coch_tab_ft = flextable::width(coch_tab_ft,j=3,  width = 3)
...
```{r MARSSSingleCovars, echo = F, warning = F, fig.height = 8, fig.cap = "Results of MARSS to predict log-transformed Chinook outcomes with Z-scored hydrologic metrics.\\label{fig:MARSSSingleCovars}"}
...
caption_text = paste0("Characteristics of MARSS models of", yvlt$y_val_title[yvlt$y_val==y_val], "using one hydrologic metric as the single covariate in each model.")
afs_ch_tab_ft = flextable::width(afs_ch_tab_ft,j=1,  width = 2)
