...
rch1_geom = geom(rch1)
conf_x = rch1_geom[nrow(rch1_geom), 4]
conf_y = rch1_geom[nrow(rch1_geom), 5]
conf_coords = data.frame(x = conf_x, y = conf_y, name = "Scott-Klamath \n Confluence")
scott_klamath_confluence=SpatialPointsDataFrame(coords = conf_coords[,1:2], 
                                                proj4string = crs(riv),
                                                data = conf_coords)
...
qvir = gIntersection(tribal, watershed) #exclude trust land outside watershed
...
cities_centroid = gCentroid(cities, byid=T)
cities_centroid = SpatialPointsDataFrame(cities_centroid, data.frame(name = cities$NAME))
# add Callahan
cal_latlong = t(matrix(data = c(-122.80142144655451, 41.309662937183475)))
cal_centroid = SpatialPointsDataFrame(coords = cal_latlong, data = data.frame(name="Callahan"))
proj4string(cal_centroid) <- CRS("+init=epsg:4326")
cal_centroid = spTransform(cal_centroid, crs(cities_centroid))
cities_centroid = rbind(cities_centroid, cal_centroid)
...
# groundwater data
scott_well_codes = wells$well_code[wells$basin_name == "Scott River Valley"]
scott_wl = wl_obs[wl_obs$well_code %in% scott_well_codes,]
...
ca_or_figure = function(include_legend = T){
  buf = buffer(ca_or, width = 3000000)
...
save_setting_figure = function(){
  ...
  main_map =  tm_shape(hill_wsh) +
    ...
    tm_shape(fj_gauge, name = "FJ Gauge") + tm_symbols(color_gauges, size = 1) +
      tm_text("station_nm", size = 1, xmod = 3, ymod = 1, fontface = "bold") +
  ...
  # produce map
  graphic_filename = file.path(diss_dir,"Graphics and Supplements","scott valley setting.png")
...
re_and_disconnect_date_tab=function(thresholds = c(10,20,30,40,60,100), fj_flow){
...
re_and_discon_timeseries_figure = function(){
  connect_tab = re_and_disconnect_date_tab(fj_flow = fj_flow)
...
calc_hbf_tab_feb2022 = function(thresholds_hbf = c(10,100), last_wy = 2021,
  ...
  if(ch1_hbftab ==T){
    write.csv(x = hbf_tab, quote = F, row.names = F,
              file = file.path(diss_dir, "Graphics and Supplements",
...
watershed_population_calc = function(){
  ...
  block_pop = read.csv(file.path(diss_dir, "Data","ScottCensusBlocksPopulation2020_DECENNIALPL2020.P1_2022-01-04T150447",
...
fj_flow_figure_old=function(){
  ...
  # Plot 2. groundwater levels?
  
  # plot(scott_wl$date, scott_wl$gse_gwe, 
  #      col = factor(scott_wl$well_code), ylim = c(50,0))
